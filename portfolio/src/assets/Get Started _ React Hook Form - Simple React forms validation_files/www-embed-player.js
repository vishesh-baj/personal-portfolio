...
l.zb=function(a){ne(this.i);this.i=a};function Sv(){return B("yt.abuse.playerAttLoader")||null}
function Tv(){Nv.apply(this,arguments)}
t(Tv,Nv);Tv.prototype.Db=function(){this.state=8};
Tv.prototype.pb=function(){return Sv()};
Tv.prototype.zb=function(a){ne(Sv());z("yt.abuse.playerAttLoader",a)};var Uv=new Tv;var Vv=new Nv;function Wv(){return M("use_player_abuse_bg_library")?Uv.isInitialized():Vv.isInitialized()}
...
var gw;t(fw,ew);fw.prototype.m=function(){if(M("web_fp_via_jspb")){var a=new qj,b=ms();b&&a.V(b);b=new Qj;Xd(b,qj,380,Rj,a);ur(b);M("web_fp_via_jspb_and_json")&&sm("finalPayload",{csn:ms()})}else sm("finalPayload",{csn:ms()})};
...
t(lw,J);lw.prototype.sendMessage=function(a,b){if(b=b||this.l){this.sessionId&&(a.id=this.sessionId);this.channel&&(a.channel=this.channel);try{var c=JSON.stringify(a);b.postMessage(c,this.targetOrigin)}catch(d){Mk(d)}}};
...
function Ew(a){var b=Fw(a),c=document.getElementById(b),d=c&&yv(c,"loaded");d||c&&!d||(c=Gw(a,b,function(){yv(c,"loaded")||(wv(c),Cq(b),el(Xa(Dq,b),0))}))}
...
l.cancel=function(){this.G&&Iv(Tw(this),this.G);fl(this.Da);this.P=!1};
...
;var bx={},cx="player_uid_"+(1E9*Math.random()>>>0);function dx(a,b,c){var d="player";c=void 0===c?!0:c;d="string"===typeof d?gf(d):d;var e=cx+"_"+Ra(d),f=bx[e];if(f&&c)return ex(a,b)?f.api.loadVideoByPlayerVars(a.args||null):f.loadNewVideoConfig(a),f.api;f=new Jw(d,e,a,b);bx[e]=f;Cq("player-added",f.api);qe(f,function(){delete bx[f.getId()]});
return f.api}
...
function lx(){fx&&fx.sendAbandonmentPing&&fx.sendAbandonmentPing();L("PL_ATT")&&(M("use_player_abuse_bg_library")?Uv.dispose():Vv.dispose());for(var a=Vh,b=0,c=Jv.length;b<c;b++)a.fa(Jv[b]);Jv.length=0;Hv("//static.doubleclick.net/instream/ad_status.js");Kv=!1;Ck("DCLKSTAT",0);oe(hx,gx);fx&&(fx.removeEventListener("onVideoDataChange",ix),fx.destroy())}
...
"WEB_PLAYER_CONTEXT_CONFIG_ID_EMBEDDED_PLAYER"in c){c=c.WEB_PLAYER_CONTEXT_CONFIG_ID_EMBEDDED_PLAYER;if(!c.serializedForcedExperimentIds){var d=Xk(window.location.href);d.forced_experiments&&(c.serializedForcedExperimentIds=d.forced_experiments)}fx=dx(a,c,!1)}else fx=dx(a);fx.addEventListener("onVideoDataChange",ix);fx.addEventListener("onReady",kx);a=L("POST_MESSAGE_ID","player");L("ENABLE_JS_API")?hx=new tw(fx):L("ENABLE_POST_API")&&"string"===typeof a&&"string"===typeof b&&(gx=new Aw(window.parent,
a,b),hx=new xw(fx,gx.connection));Lv();M("ytidb_create_logger_embed_killswitch")||pm();a={};gw||(gw=new fw);gw.install((a.flush_logs={callback:function(){Wq()}},a));
